...
To get the number of rows for each review score earned by 'Hotel Arena', we can write an SQL query to count the occurrences of each `reviewer_score` for the hotel 'Hotel Arena'. Here's how you can do it:
---
...
* **SELECT hotel\_name, reviewer\_score**: This selects the hotel name and the review score from the table.
* **COUNT(\*)**: This counts the number of rows for each `reviewer_score`.
* **FROM hotel\_reviews**: Specifies the table `hotel_reviews`.
* **WHERE hotel\_name = 'Hotel Arena'**: Filters the rows for only the reviews of 'Hotel Arena'.
* **GROUP BY reviewer\_score, hotel\_name**: Groups the results by `reviewer_score` and `hotel_name` so you can count the rows for each score.

### Expected Output:

This query will output the hotel name, the review score, and the number of rows for each review score given for 'Hotel Arena'.

For the provided data, it would give something like:

```
| hotel_name  | reviewer_score | number_of_rows |
|-------------|----------------|----------------|
| Hotel Arena | 8.0            | 1              |
| Hotel Arena | 9.0            | 1              |
| Hotel Arena | 6.0            | 1              |
...
This shows that 'Hotel Arena' has received review scores of 8.0, 9.0, and 6.0, each with 1 row.
